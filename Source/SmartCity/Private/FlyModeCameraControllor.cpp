...
//鼠标拖拽
void AFlyModeCameraControllor::DragRotation(const FVector2D inCursorPt, const FVector2D inNextCursorPt)
{
    //获取相机近裁剪面点对应的ue坐标系点
    FVector intersectPt;
...
void AFlyModeCameraControllor::LeftButtonDown()
{
    ...
    {
        VirtualRadius = (WorldPos - EarthActor->GetActorLocation()).Size();
...
void AFlyModeCameraControllor::Zoom(const float Speed)
{
    ...
    if (!LineSphereFirstIntersect(
        ...
        WorldDirection, EarthActor->GetActorLocation(), IntersectPt))
        return;
